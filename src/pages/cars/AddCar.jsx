  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      const car = {
        ...values,
        id: maxId + 1,
        photos: values.photos.map((photo) => URL.createObjectURL(photo)),
        technicalCondition: values.technicalCondition.join(", "),
        comfort: values.comfort.length > 0 ? values.comfort.join(", ") : "-",
        multimedia: values.multimedia.length > 0 ? values.multimedia.join(", ") : "-",
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Grid container spacing={2} style={{ marginTop: "10px" }}>
        ...
        <Grid item xs={12} sm={6} style={{marginTop: "10px", marginLeft: "10px"}}>
          <FormGroup>
